...
$debug=false;
 ...
 if(isset($_POST['submit'])){
     ...
     $array_id_old =$_POST['id_tipo_immobile'];
     echo "new name: ".$newName . "<br>";
     
      $id_tipo_immobile_new = inserisci_tipo_immobile($newName);
      echo '$id_tipo_immobile_new '.  $id_tipo_immobile_new . " <br>";
      $db = new db(DB_USER,DB_PASSWORD,DB_NAME,DB_HOST);
    $query="SELECT * FROM immobile ";
    $r=$db->query($query);
    while($row=  mysql_fetch_array($r)){
        if(in_array($row['id_tipo_immobile'], $array_id_old)){
            //SOSTITUISCO
            echo "id del db: ".$row['id_tipo_immobile'];
            echo " - trovato nell'array";
            echo "<br>";
            if(!$debug){
             $db2 = new db(DB_USER,DB_PASSWORD,DB_NAME,DB_HOST);
             $query2="UPDATE immobile SET id_tipo_immobile = $id_tipo_immobile_new  WHERE id_imm =".$row['id_imm'];
    ...
    foreach ($array_id_old as $id_to_delete){
        if(!$debug){
           $db3 = new db(DB_USER,DB_PASSWORD,DB_NAME,DB_HOST);
             $query3="DELETE FROM tipo_immobile WHERE id_tipo_immobile =". $id_to_delete;
	...
	<div id="center">
	   ...
	   <p>Attenzione tutte le modifiche saranno applicate a tutti gli immobili e clienti agganciati con la rispettiva citta</p>
        	...
        	echo '<form method="post">';
                ...
                echo '<ul>';
                $new_name ="";
foreach ($_POST["id_tipo_immobile"] as $selectedOption){
    $query= "select * from tipo_immobile where id_tipo_immobile = ".$selectedOption;
    $r=$db->query($query);
    $ro=  mysql_fetch_array($r);
    echo '<li>';
    echo $selectedOption ." " . $ro["nome"];
    echo '<input type="hidden" name="id_tipo_immobile[]" value="'.$selectedOption.'"/>';
    echo '</li>';
    $new_name=$ro["nome"];
